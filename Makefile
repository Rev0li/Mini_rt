...
# Liste complète des fichiers sources de la MLX
...
HEADER = include/mini_rt.h include/struct.h
...
# ─── Object compilation with dependencies ────────────────────────────
$(OBJDIR)/%.o: src/%.c $(HEADER) $(MLX_DIR)/mlx.h
	@mkdir -p $(dir $@)
	@mkdir -p $(dir $(DEPDIR)/$<)
	$(CC) $(CFLAGS) -MMD -MP -c $< -o $@ -MF $(DEPDIR)/$(basename $<).d
...
# ─── Include .d files for header dependencies ────────────────────────
-include $(DEP)
